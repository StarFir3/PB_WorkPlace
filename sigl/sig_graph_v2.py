    ...
    for row in df.itertuples(index=False):
        ...
        if G.has_node(process_id) or G.has_node(row.Dst_Addr):
            ...
            # If TcpIp/Send, the edge should be Src_Addr to Dst_Addr
                ...
                src = row.Dst_Addr
                dst = process_id
            elif row.Event_Name == 'TcpIp/Connect':
            # If Process_Stop, the edge should be Process_Id to Parent_Id
                src = process_id
                dst = row.Dst_Addr
...
def task_fileio():
    ...
    df = pd.read_csv(r'csv_files/FileileIO.csv',  header = 0)
    # Reversing the CSV
    df = df.iloc[::-1]    

    # Process first row to get the objects
    row = df.iloc[0]
    src = row["Process_Name"]
    dst = row["File_Path"]

    # If parent_id and process_id node doesn't exists create it
    if not G.has_node(src):
        G.add_node(src, Time = row["Time"] )
    if not G.has_node(dst):
        G.add_node(dst, Time = row["Time"])
